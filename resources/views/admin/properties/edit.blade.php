...
@section('content')
    ...
    <div class="content">
        ...
        <div class="container-fluid pt-4 px-4">
            <div class="unit-form-page pb-5">
                ...
                <form id="adminPropertyForm" action="{{ route('admin.properties.update', $property->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="card unit-form-card shadow-sm mb-4">
                        ...
                        <div class="col-lg-8 mb-3">
                            ...
                            <textarea name="description" class="form-control summernote @error('description') is-invalid @enderror"
                                      id="propertyDescription" rows="6" data-height="220">{!! old('description', $property->description) !!}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="card unit-form-card shadow-sm mb-4">
                        <div class="card-body p-4">
                            <h5 class="fw-semibold mb-3">Terms and conditions</h5>
                            <p class="text-muted small mb-3">Cancellation policy, refund rules, no-show policy, house rules, and check-in expectations — shown to guests on the public property page.</p>
                            @php
                                $listingTermsValue = old('listing_terms');
                                if ($listingTermsValue === null) {
                                    $listingTermsValue = $property->resolvedListingTerms();
                                }
                            @endphp
                            <div class="col-12">
                                <label for="propertyListingTerms" class="form-label">Terms and conditions</label>
                                <textarea name="listing_terms" class="form-control @error('listing_terms') is-invalid @enderror" id="propertyListingTerms" rows="10">{!! $listingTermsValue !!}</textarea>
                                @error('listing_terms')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
    ...
    <script>
        $(document).ready(function() {
            $('#adminPropertyForm').on('submit', function () {
                ['#propertyDescription', '#propertyListingTerms'].forEach(function (sel) {
                    var $ta = $(sel);
